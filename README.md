...
| 🤖 ML (LSTM) | 神經網路 → Top-K 機率連乘 | 全部 C(39,k) | 看 ML 跟統計法差異 | 需先訓練,首次載 ~3MB |
...
**組合分數**:取機率前 12 名號碼,枚舉 C(12, k) 組合,每組分數 = 各號機率連乘

**訓練**:GitHub Actions Python pipeline,在 Ubuntu CPU 上跑 ~10-15 分鐘 (依 EarlyStopping)

**評估方式**:
- 切 70/15/15 訓練/驗證/測試
- 對 test set 做 Top-K 命中數 (K=5)
- 對隨機抽樣模擬 (paired t-test)
- 報告 p-value + 95% CI

**獨立 ML tab** 顯示完整評估 (模型平均命中 vs 隨機 / p-value / 命中分布柱狀圖 / 下期 Top 10 號碼)。

**預期結果**:539 為 IID 隨機事件,模型表現應**收斂到隨機基準** (p > 0.05)。p-value 顯著反而值得懷疑(可能是資料洩漏或抽樣巧合)。
